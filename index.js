...
const commands = [
    ...
    new SlashCommandBuilder().setName('miningrate').setDescription('Displays the current mining rates.'),
    new SlashCommandBuilder().setName('exchanges').setDescription('Get a list of verified exchanges where Pi is available.'),
    new SlashCommandBuilder().setName('balance').setDescription('Check wallet balance by providing a wallet address.')
...
client.on('interactionCreate', async (interaction) => {
    ...
    if (commandName === 'price') {
        ...
        const embed = new EmbedBuilder()
            .setTitle('Help Menu')
            .setDescription('`/price`, `/help`, `/ping`, `/miningrate`, `/exchanges`, `/fdv`, `/volume`, `/balance`')
            .setColor(0x4C2F71);
        await interaction.reply({ embeds: [embed] });
        ...
        const embed = new EmbedBuilder()
            .setTitle('KYB-verified businesses')
            .setDescription(`**A list of KYB-verified platforms where Pi is available can be found here:**
https://minepi.com/kyb-list/#VerifiedBusinesses`)
            .setColor(0x4C2F71);
        await interaction.reply({ embeds: [embed] });
